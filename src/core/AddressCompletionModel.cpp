	...
	if (m_types.testFlag(LocalPathSuggestionsCompletionType) && m_filter.contains(QDir::separator()))
	{
		...
		for (int i = 0; i < entries.count(); ++i)
		{
			if (entries.at(i).fileName().startsWith(prefix, Qt::CaseInsensitive))
			{
				...
				completions.append(CompletionEntry(QUrl(QDir::fromNativeSeparators(path)), path, path, QIcon::fromTheme(type.iconName(), iconProvider.icon(entries.at(i))), LocalPathType));
			}
		}
	}
